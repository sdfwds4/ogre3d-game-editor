...
LRESULT CMainFrame::OnDockingPaneNotify(WPARAM wParam, LPARAM lParam)
{
	if (wParam == XTP_DPN_SHOWWINDOW)
	{
		...
		if (!pPane->IsValid())
		{
			...
			case IDR_PANE_PROPERTIES:
				{
					if (m_wndLogEdit.GetSafeHwnd() == 0)
					{
						m_wndLogEdit.Create(WS_CHILD | WS_VSCROLL| ES_AUTOVSCROLL | ES_READONLY | ES_WANTRETURN | ES_MULTILINE,
							CRect(0, 0, 0, 0), this, 0);
					}
					pPane->Attach(&m_wndLogEdit);

					GetEditor()->SetLogWindow( &m_wndLogEdit );
					break;
				}
			case IDR_PANE_OPTIONS:
				{
					pPane->AttachView( this, RUNTIME_CLASS(CMainToolView) );
					break;
				}
			}
		}
	...
	{
		CDialog* pStatus = GetEditor()->GetStatusDialog();
		if ( pStatus )
		{
			pStatus->Invalidate();
		}
